...
// AILinkSummaryStore represents a store for managing AI-generated link summaries
type AILinkSummaryStore interface {
	// Save creates a new link summary
	Save(summary *model.AILinkSummary) (*model.AILinkSummary, error)

	// Get retrieves a link summary by ID
	Get(id string) (*model.AILinkSummary, error)

	// GetByURLHash retrieves the latest unexpired summary for a URL hash
	GetByURLHash(urlHash string) (*model.AILinkSummary, error)

	// DeleteExpired removes expired link summaries
	DeleteExpired(currentTime int64) (int64, error)

	// Delete removes a specific link summary
	Delete(id string) error
}
